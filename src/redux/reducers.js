...
function user(state=initUser, action) {
  switch (action.type) {
    case AUTH_SUCCESS: // data是user
      ...
      const redirect = getRedirectTo(type, header)
      console.log('redirect', redirect)
      return {...action.data, msg: ' 操作成功！', redirectTo: redirect}
...
function chat(state=initChat, action) {
  switch (action.type) {
    case RECEIVE_MSG_LIST:  // data: {users, chatMsgs}
      ...
      return {
        chatMsgs,
        unReadCount: 6 // chatMsgs.reduce((preTotal, msg) => preTotal+(!msg.read&&msg.to===userid?1:0),0)
      }
